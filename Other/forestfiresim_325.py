...
Modified By: Wade Eckert, Scott Johnson, Zak Nizam
Date Modified: 24 January 2026
Description: A simulation of wildfires spreading in a forest. Press Ctrl-C to stop.
This version includes a lake in the center of the forest that fire cannot spread into.
======================================================================================"""
        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                if ((forest[(x, y)] == EMPTY)
                    ...
                    for ix in range(-1, 2):
                        for iy in range(-1, 2):
                            # Fire spreads to neighboring trees (but not to water):
...
def createNewForest():
    ...
    # Add a lake in the center of the display
    addLake(forest)
    return forest


def addLake(forest):
    """Create a rectangular lake in the center of the forest that is roughly 1/3 the size of the forest."""
    lake_width = forest['width'] // 3
    lake_height = forest['height'] // 3
    start_x = (forest['width'] - lake_width) // 2
    start_y = (forest['height'] - lake_height) // 2
    
    # Create a rectangular lake
